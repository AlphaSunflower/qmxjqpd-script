...
class ChaolianStepStrategy(BaseStrategy):
    """
    超级联赛天梯赛策略
    """
    ...
    def _inner_match(self):
        self.loading = None
        self.status = "FREE"  # FREE空闲、MATCHING正在匹配、INNER正在比赛、ENDING结算中
            ...
            if self.status == "ENDING":
                if self._match_sift("chaolian_win.png", 50):
                    if self._ocr((950, 620, 70, 30), "返回"):
                        ...
                        logger.info(
                            "比赛胜利,胜利" + str(self.victory_count) + "场" + "失败" + str(self.failure_count) + "场")
                ...
                if self._match_sift("chaolian_defeat.png", 40):
                    if self._ocr((950, 620, 70, 30), "返回"):
                        ...
                        logger.info(
                            "比赛失败,失败" + str(self.failure_count) + "场" + "胜利" + str(self.victory_count) + "场")
                        self._tap_with_offset(920, 635, offset=5)
                        self.status = "FREE"
                        self.match_count += self.change_count
                        logger.info(f"比赛结束，场次计数：{self.match_count}")
