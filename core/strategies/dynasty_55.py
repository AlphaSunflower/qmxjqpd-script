            ...
            if self.status == "SEARCHING":
                ...
                if self._ocr((910,405,140,25),"全场") or self._match_sift("dynasty_55.png", 50):
                    ...
                    if self._match_sift("mode_choose.png",50):
                        ...
                        self._swipe(1000, 570, 1000, 0, 500)
                        self._sleep(1)
                        pos = self._match_sift("dynasty_55.png", 50)
                        if pos:
                            self._tap_with_offset(pos[0], pos[1], offset=5)
                            self._sleep(1)
                            self._tap_with_offset(1000, 650, offset=5)
                            self._sleep(2)
                            made_progress = True
                        else:
                            logger.error("未找到王朝55模式,请联系开发人员")
                    else:
                        self._sleep(1)
                self._sleep(1)
                ...
                if self.detecting_hall():
                    ...
                    self._sleep(1)
                    made_progress = True
                elif self._match_sift("over.png",120):
                    self.status = "OVER"
                    made_progress = True
                else:
                    if self._match_sift("inner.png",50):
                        ...
                        logger.debug("已经进入对局")
                        made_progress = True
                    elif self._match_sift("victory.png", 50) or self._match_sift("defeat.png", 50):
                ...
                if self._match_sift("inner.png", 50) or self._match_sift("game.png", 50):
                    if self.sending_drawing and not self.is_sending_drawing:
                        ...
                        self._sleep(2)
                    made_progress = True
                elif self._match_sift("55_ready.png",50):
                    self._tap_with_offset(780, 610, offset=2)
                    self._sleep(2)
                    made_progress = True
                ...
                if self._match_sift("over.png",120):
                    ...
                    if self._match_sift("victory.png", 50) or self._match_sift("defeat.png", 50):
                        logger.info(f"对局结束，胜利{self.victory_count}场，失败{self.failure_count}场")
                        self._tap_with_offset(990, 640, offset=2)
                        self._sleep(2)
                        self.status = "FREE"
